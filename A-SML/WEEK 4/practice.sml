...
fun f2 xs = ((fold (x,y)=> x andalso y >=0), true, xs);

(* number of elements between lo and hi *)
fun f3 (xs, lo, hi) =
    fold ((fn (x,y) =>
	      x + (if y >=lo andalso y<=hi then 1 else 0)),
	  0, xs);

(* check all strings lower than size of given string s *)
fun f4 (xs, s) =
